...
const tabs = [
  ...
  { id: "organization", label: "Organization", icon: Building2, desc: "Clinic details & staff seats" },
  { id: "appearance", label: "Appearance", icon: Palette, desc: "Theme preferences & density" },
        ...
        <div className="mt-6">
          ...
          {activeTab === "organization" && (
            <div className="space-y-6">
              <div className="rounded-2xl border border-border/50 bg-card p-6 shadow-xs">
                <div className="border-b border-border/40 pb-4 mb-6">
                  <h2 className="text-[15px] font-bold text-foreground tracking-tight">Clinic Profile & Practice Entity</h2>
                  <p className="text-[12px] text-muted-foreground mt-0.5">Official legal details for billing and patient invoices.</p>
                </div>

                <div className="space-y-1 divide-y divide-border/30">
                  <FormRow label="Clinic Brand Name" description="Displayed at top of app and patient portal.">
                    <input
                      type="text"
                      defaultValue="Meridian Medical Clinics"
                      className="h-10 w-full rounded-xl border border-border/60 bg-background px-3.5 text-[12.5px] text-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
                    />
                  </FormRow>

                  <FormRow label="Legal Entity & Tax ID" description="For official invoices & tax compliance.">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <input
                        type="text"
                        defaultValue="Meridian Healthcare GmbH"
                        className="h-10 w-full rounded-xl border border-border/60 bg-background px-3.5 text-[12.5px] text-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
                        placeholder="Legal Entity Name"
                      />
                      <input
                        type="text"
                        defaultValue="DE 342 118 990"
                        className="h-10 w-full rounded-xl border border-border/60 bg-background px-3.5 text-[12.5px] text-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
                  ...
                  <FormRow label="Clinic Address" description="Primary physical location of the facility.">
                    <textarea
                      rows={2}
                      defaultValue="Torstraße 142, 10119 Berlin, Germany"
                      className="w-full rounded-xl border border-border/60 bg-background p-3 text-[12.5px] text-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all resize-none"
                    />
                  </FormRow>
                </div>
              ...
              <div className="rounded-2xl border border-border/50 bg-card p-6 shadow-xs">
                <div className="flex items-center justify-between border-b border-border/40 pb-4 mb-4">
                  <div>
                    <h3 className="text-[14px] font-bold text-foreground">Clinic Staff Seats</h3>
                    <p className="text-[12px] text-muted-foreground mt-0.5">4 active practitioner seats used out of 10 available.</p>
                  </div>
                  <Button size="sm">+ Invite Staff</Button>
                </div>
                <div className="divide-y divide-border/30">
                  {[
                    { name: "Dr. Iman Reyes", role: "Medical Director (Owner)", email: "iman@meridian.io" },
                    { name: "Dr. Ngozi Okafor", role: "Consultant Physician", email: "ngozi@meridian.io" },
                    { name: "Lukas Berger", role: "Front Desk Administrator", email: "lukas@meridian.io" },
                    { name: "Amelia Voss", role: "Practice Administrator", email: "amelia@meridian.io" },
                  ].map((staff) => (
                    <div key={staff.email} className="flex items-center justify-between py-3">
                      <div className="flex items-center gap-3">
                        <div className="h-9 w-9 rounded-xl bg-emerald-500/10 text-emerald-800 dark:text-emerald-300 font-bold text-[12px] flex items-center justify-center">
                          {staff.name.split(" ").map(n => n[0]).slice(0, 2).join("")}
                        </div>
                        <div>
                          <div className="text-[13px] font-semibold text-foreground">{staff.name}</div>
                          <div className="text-[11px] text-muted-foreground font-mono">{staff.role} · {staff.email}</div>
                        </div>
                      </div>
          ...
          {activeTab === "appearance" && (
            <div className="space-y-6">
              <div className="rounded-2xl border border-border/50 bg-card p-6 shadow-xs">
                <div className="border-b border-border/40 pb-4 mb-6">
                  <h2 className="text-[15px] font-bold text-foreground tracking-tight">Theme & Interface Customization</h2>
                  <p className="text-[12px] text-muted-foreground mt-0.5">Customize the visual styling of your clinic dashboard.</p>
                </div>

                <div className="space-y-6">
                  <div>
                    <label className="text-[13px] font-semibold text-foreground block mb-3">Color Mode</label>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      {[
                        { id: "light", label: "Light Mode", desc: "Clean bright aesthetic" },
                        { id: "dark", label: "Dark Mode", desc: "Sleek high contrast" },
                        ...
                        >
                          <div className="h-16 rounded-xl border border-border/40 bg-muted/40 mb-3 flex items-center justify-center">
                          ...
                          <div className="flex items-center justify-between text-[13px] font-semibold text-foreground">
                            {t.label}
                            {theme === t.id && <Check className="h-4 w-4 text-emerald-600" />}
                          </div>
                  ...
                  <div className="pt-4 border-t border-border/30">
                    <label className="text-[13px] font-semibold text-foreground block mb-3">Layout Density</label>
                    <div className="flex items-center gap-3">
                      {[
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-[12.5px] font-medium border transition-all cursor-pointer ${
                            density === d.id
